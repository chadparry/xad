...
def find_chessboard_corners(image):
	...
	for block_size in (2**exp - 1 for exp in itertools.count(5)):
		...
		for dilation in [4, 1]:
			...
			contoursd, hierarchy = cv2.findContours(dilated,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
			contourse, hierarchy = cv2.findContours(eroded,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
