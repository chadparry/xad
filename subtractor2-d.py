...
stable = first
stableg = cv2.cvtColor(stable, cv2.COLOR_BGR2GRAY)
...
while(1):
    ...
    offset = iter(history)
    next(offset)
    for (prevf, f) in zip(history, offset):
    #for f in history:
        ...
        if placements is None:
            ...
            grayprevf = cv2.cvtColor(prevf, cv2.COLOR_BGR2GRAY)
            flow = cv2.calcOpticalFlowFarneback(grayprevf, grayf, None, 0.5, 3, 15, 3, 5, 1.2, 0)
            movedmag, ang = cv2.cartToPolar(flow[...,0], flow[...,1])
            (ret, movedmaskf) = cv2.threshold(movedmag, 1, 255, cv2.THRESH_BINARY_INV)
            movedmask = movedmaskf.astype(numpy.uint8)
